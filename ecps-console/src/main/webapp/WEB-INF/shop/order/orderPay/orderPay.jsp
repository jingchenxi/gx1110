...
<body id="main">
	...
	<div class="frameR">
		<div class="content">
			<div class="loc icon">
				<samp class="t12"></samp>
				当前位置：订单管理&nbsp;&raquo;&nbsp;<span class="gray" title="待付款单">待付款单</span>
			</div>
            <input type="hidden" id="isCall" value="${isCall }"/> 
			<h2 class="h2_ch">
				<span id="tabs" class="l"> 
				<a id="label1" 
				href="${path}/order/listNoPay.do?assignee=noPaidOrderer"
					title="全部" class="nor">全部</a> <a id="label2"
					href="${path}/order/listNoPay.do?isCall=0&assignee=noPaidOrderer"
					title="未付款待外呼" class="nor">未付款待外呼</a> <a id="label3"
					href="${path}/order/listNoPay.do?isCall=1&assignee=noPaidOrderer"
					title="已外呼" class="nor">已外呼</a> <%--         <a id="label4" href="${path}/order/orderPay.do?orderState=17"  title="超时作废" class="nor">超时作废</a> --%>
				</span>

			</h2>
			<form action="${path}/order/orderPay.do" id="form1" name="form1"
				method="post">
				...
				<table cellspacing="0" summary="" class="tab" id="myTable">
					...
					<c:forEach items="${tbList }" var="tb">
						<tr>
							<td><a
								href="${path }/shop/order/orderPay/detail.jsp?orderId=3123">${tb.order.orderNum }</a></td>
						...
						<td>
							...
							<td><a
								href="${path }/order/selectOrderAndDetail.do?orderId=${tb.order.orderId}">查看</a></td>
						</tr>
					</c:forEach>
