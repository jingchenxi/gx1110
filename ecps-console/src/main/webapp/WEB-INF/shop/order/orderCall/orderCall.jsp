...
<div class="frameR"><div class="content">
    ...
    <h2 class="h2_ch">
      <span id="tabs" class="l">
       <%--  <a id="label6" href="${path}/order/orderCall.do"   title="全部" class="nor">已付款单</a>
        ...
        <a id="label5" href="${path}/order/orderCall.do?orderState=15,26,33,34,10,11"  title="外呼作废" class="nor">已付款单</a>
      </span>
      <span class="r inb_a">
				...
				<table cellspacing="0" summary="" class="tab" id="myTable">
					...
					<c:forEach items="${tbList }" var="tb">
						...
						<td>
							...
							<td class="nwp">
							  <c:if test="${tb.order.isPaid == 1 }">已付款单</c:if>
							<c:if test="${tb.order.isPaid == 0 }">未付款单</c:if>
							</td>
							<td><a
								href="${path }/order/selectPaidOrderDetail.do?orderId=${tb.order.orderId}&taskId=${tb.task.id}">查看</a></td>
						</tr>
					</c:forEach>
